...
# LF mapped to 127-30
def decodeChar(c, avoid_tab_and_lf=False):
    """Decode a code point
    :param c: code point
...
def encode_text(s):
    ...
    """
    return list(map(lambda a: decodeChar(ord(a)), s))


def decode_text(c, avoid_tab_and_lf=False):
    """Decode an encoded string.
    :param c: encoded list of code points
    :param avoid_tab_and_lf: if True, tab and line feed characters are replaced by '\'
    :return:
    """
    return "".join(map(lambda a: chr(decodeChar(a, avoid_tab_and_lf)), c))
...
def read_data_files(directory, validation=True):
    ...
    for cardfile in cardlist:
        ...
        start = len(codetext)
        codetext.extend(encode_text(cardtext.read()))
        end = len(codetext)
